...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const PRODUCT_API_BASE_URL = "http://4.254.74.192:8000/";
  const ORDER_API_BASE_URL = "http://4.198.17.51:8001/";
  ...
  async function fetchProducts() {
    ...
    try {
      ...
      productListDiv.innerHTML =
        "<p>Could not load products. Please check the Product Service.</p>";
    }
  }
  ...
  productForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        `Product "${addedProduct.name}" added successfully! ID: ${addedProduct.product_id}`,
        "success"
      );
  ...
  productListDiv.addEventListener("click", async (event) => {
    ...
    if (event.target.classList.contains("upload-btn")) {
      ...
      try {
        ...
        const response = await fetch(
          `${PRODUCT_API_BASE_URL}products/${productId}/upload-image`,
          {
            method: "POST",
        ...
        showMessage(
          `Image uploaded successfully for product ${updatedProduct.name}!`,
          "success"
        );
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item) => {
        ...
        li.innerHTML = `
                    ...
                    <span>${formatCurrency(item.price)} each - ${formatCurrency(
          itemTotal
        )}</span>
                `;
  ...
  placeOrderForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      showMessage(
        ...
        } placed successfully! Total: ${formatCurrency(
    ...
    try {
      ...
      orders.forEach((order) => {
        ...
        orderCard.innerHTML = `
                    ...
                    <p>Status: <span id="order-status-${order.order_id}">${
          order.status
        }</span></p>
                    ...
                    <div class="status-selector">
                        ...
                        <button class="status-update-btn" data-id="${
                          order.order_id
      ...
      orderListDiv.innerHTML =
        "<p>Could not load orders. Please check the Order Service.</p>";
    }
  }
  ...
  orderListDiv.addEventListener("click", async (event) => {
    ...
    if (event.target.classList.contains("status-update-btn")) {
      ...
      try {
        showMessage(
          `Updating status for order ${orderId} to "${newStatus}"...`,
          "info"
        );
        const response = await fetch(
          `${ORDER_API_BASE_URL}orders/${orderId}/status?new_status=${newStatus}`,
          {
            method: "PATCH",
          }
        );
        ...
        document.getElementById(`order-status-${orderId}`).textContent =
          updatedOrder.status;
        showMessage(
    ...
    if (event.target.classList.contains("delete-btn")) {
      ...
      if (
        !confirm(
          `Are you sure you want to delete order ID: ${orderId}? This will also delete all associated items.`
        )
      ) {
        return;
      }
